...
class Adapter:
    ...
    def execute(self, command):
        ...
        if "type" in command:
            if command["type"] == "search":
                if "term" in command:
                    status, fehler, trefferzahl = self.suche(
                        command["term"], formatted_filters
                    )
                    if status == "ok":
                ...
                if "term" in command:
                    ...
                    status, fehler, trefferliste = self.treffer(
                        command["term"], formatted_filters, start, count
                    )
                    if status == "ok":
                        return {
                            "status": "ok",
                            "hitlist": trefferliste,
                            "start": start,
    ...
    def format_filters(self, filters):
        ...
        for filter in filters:
            if filter["type"] == "checkbox":
                ...
                ):  # only consider filter when at least one checkbox is checked
                ...
                if filter["from"] != "" or filter["to"] != "":
                    formatted_filters.append(
                        filter
                    )  # don't add year filter if values are empty
    ...
    def get_cachekey(self, suchstring, filters):
        ...
        for filter in filters:
            if filter["type"] == "checkbox":
                stringified_filters.append(
                    filter["id"] + "=" + ",".join(filter["options"])
                )
            elif filter["type"] == "year":
                stringified_filters.append(
                    "from=" + str(filter["from"]) + "&to=" + str(filter["to"])
                )
    ...
    def suche(self, suchstring, filters):
        ...
        if cachekey in self.cache:
            if (
                datetime.datetime.now() - self.cache[cachekey].zeit
            ).total_seconds() < 86000:
    ...
    def treffer(self, suchstring, filters, von=0, zahl=10):
        ...
        if cachekey in self.cache:
            if (
                ...
                del self.cache[cachekey]
                fehler = self.request(
                    suchstring, filters, von, min(self.LISTSIZE, zahl)
                )
                if fehler:
                    return "error", fehler, []
        else:
        ...
        if trefferzahl <= von and von > 0:
            return (
                "error",
                "Search has only "
                + str(self.cache[cachekey].trefferzahl)
                + " Hits. Cannot serve a hitlist starting with hit "
                + str(von)
                + ".",
                [],
            )
            ...
            while i < min(von + zahl, trefferzahl):
                if i not in treffercache:
                    fehler = self.request(
                        suchstring, filters, i, max(self.LISTSIZE, zahl - (i - von))
                    )
                    if fehler:
                        return "error", fehler, []
                    treffercache = self.cache[cachekey].trefferliste
