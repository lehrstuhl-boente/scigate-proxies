    ...
    def request(self, suchstring, filters="", start=0, count=Adapter.LISTSIZE):
        ...
        form_data = {
            ...
            "asp_inst_id": (
                None,
                "1_1",
            ),  # search instance ID --> one search can have multiple instances
            "options": (
                None,
                "filters_initial=1&filters_changed=0&qtranslate_lang=0&current_page_id=13",
            ),  # serialized string of search form options
            "asp_call_num": (None, page),  # which page of the results
        }
        response = requests.post(
            url=self.host + self.suchpfad, files=form_data, headers=self.headers
        )
