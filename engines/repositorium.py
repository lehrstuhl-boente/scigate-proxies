        ...
        for dokument in rs:
            ...
            if coauthors is not None:
                for author in coauthors:
                    coauthorsArray.append(
                        author["coauthor_first"] + author["coauthor_last"]
                    )
            else:
            ...
            zeile3 = (
                "Publish date: "
                + dokumentAnswer["Erschienen_am"]
                + " | "
                + dokumentAnswer["Erschienen_in"]
            )
            trefferliste.append(
                {
                    "engineId": self.id,
                    "description": [zeile1, zeile2, zeile3],
                    "url": "https://dev.repositorium.ch/Entry/"
                    + str(dokumentAnswer["id"]),
                }
            )
